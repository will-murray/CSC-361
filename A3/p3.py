...
class IP_Datagram():
    ...
    def __init__(self,buffer,incl_len,ts_sec, ts_usec):
        ...
        self.header = IP_Header(buffer)
        self.payload = buffer[14 + self.header.header_length:] #ethernet header length + IP header length

        if self.header.proto == 17: # UDP
            udp_header = self.payload[:8]  # IP header is 20 bytes, UDP header is 8 bytes
            self.udp_fields = struct.unpack("!HHHH", udp_header)
            self.udp_src_port = self.udp_fields[0]
        
        elif self.header.proto == 6: # TCP
            pass

        else: # ICMP
            self.icmp_header = self.payload[:8]
            self.icmp_fields = struct.unpack("!BBHHH", self.icmp_header) 
            print(self.icmp_fields)
            # self.icmp_type = self.icmp_fields[0]
            # self.icmp_id = self.icmp_fields[2]
            # if self.icmp_type == 11:s
            #     self.OG_header = IP_Header(self.payload[8:28])
